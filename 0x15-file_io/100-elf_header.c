 ...
 *
 * Description:displays if the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_entry: this is address of the ELF entry point.
 * @e_ident: this is a pointer to an array containing the ELF class.
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 *
 * Description:displays If the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
